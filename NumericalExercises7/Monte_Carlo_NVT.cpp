...
//reset the hystogram of g(r)
  for (int k=igofr; k<igofr+nbins; ++k) walker[k]=0.;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open("averages/output.epot.0",ios::app);
    Pres.open("averages/output.pres.0",ios::app);
    Gofr.open("averages/output.gofr.0",ios::app);
    Gave.open("averages/output.gave.0",ios::app);
...
//g(r)
	ofstream deltav;
	deltav.open("deltav.out");
	for(int k=0; k<nbins; ++k){
		r = bin_size*k;
		dv = 4./3.*M_PI*(pow(r+bin_size, 3)-pow(r,3));
		gdir = (blk_av[k+igofr]/blk_norm)/(rho*(double)npart*dv);
		deltav << k << "	" << dv << endl;
		...
		Gofr << setw(wd) << iblk <<  setw(wd) << gdir << endl;
		if(iblk==nblk)
			Gave << setw(wd) << glob_av[k+igofr]/(double)iblk << setw(wd) << err_gdir << endl;
	}
	deltav.close();
...
void Instant(){
	ofstream out_en, out_pres;
	out_en.open("measures/instant_energy.out", ios::app);
	out_pres.open("measures/instant_pressure.out", ios::app);
	out_en << walker[iv]/(double)npart + vtail << endl;
	out_pres << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
	out_en.close();
	out_pres.close();
}
